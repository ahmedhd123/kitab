  ...
  const handleSubmit = async (e: React.FormEvent, isDraft: boolean = true) => {
    ...
    try {
      ...
      if (!formData.title.trim()) {
        showError('خطأ في البيانات', 'عنوان الكتاب مطلوب');
        return;
      }
      
      if (!formData.author.trim()) {
        showError('خطأ في البيانات', 'اسم المؤلف مطلوب');
        return;
      }
      
      if (!formData.genre) {
        showError('خطأ في البيانات', 'نوع الكتاب مطلوب');
        return;
      }

      showInfo('جاري المعالجة...', isDraft ? 'جاري حفظ الكتاب كمسودة' : 'جاري نشر الكتاب');
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        if (key === 'tags') {
          ...
          submitData.append(key, isDraft ? 'draft' : 'published');
        } else {
      ...
      if (response.ok && data.success) {
        ...
        if (isDraft) {
          showSuccess(
            'تم حفظ الكتاب بنجاح!', 
            'تم حفظ الكتاب كمسودة. يمكنك تعديله أو نشره لاحقاً.'
          );
        } else {
          showSuccess(
            'تم نشر الكتاب بنجاح!', 
            'الكتاب متاح الآن للقراء في المكتبة.'
          );
        }
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex flex-col sm:flex-row gap-4 justify-end">
              <button
                ...
                disabled={loading}
                className="bg-gray-600 text-white px-6 py-3 rounded-lg hover:bg-gray-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center"
              >
              ...
              <button
                ...
                disabled={loading}
                className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {loading && isPublishing ? (
                  <>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white ml-2"></div>
              ...
              <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div className="text-sm text-blue-800">
                    {isPublishing ? (
                      <>
                        <span className="font-medium">جاري نشر الكتاب...</span>
                        <br />
                        <span className="text-blue-600">سيتم توجيهك لصفحة تفاصيل الكتاب بعد النشر</span>
                      </>
                    ) : (
                      <>
                        <span className="font-medium">جاري حفظ المسودة...</span>
                        <br />
                        <span className="text-blue-600">يمكنك المتابعة والنشر لاحقاً</span>
                      </>
                    )}
                  </div>
                </div>
