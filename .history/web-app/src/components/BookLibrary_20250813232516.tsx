...
export default function BookLibrary() {
  ...
  useEffect(() => {
    ...
    // الحصول على الكتب المحلية
    const localBooks = getLocalBooks();
    
    // تحويل الكتب المحلية إلى تنسيق Book
    const convertedLocalBooks: Book[] = localBooks.map(localBook => ({
      ...
      fileUrl: localBook.id, // سنستخدم ID للوصول للملف المحلي
    ...
    // دمج الكتب العينة مع الكتب المحلية
    ...
    setTimeout(() => {
      ...
      if (convertedLocalBooks.length > 0) {
        console.log(`📚 تم تحميل ${convertedLocalBooks.length} كتاب محلي و ${sampleBooks.length} كتاب عينة`);
      }
    }, 1000);
  }, []);
  ...
  const handleBookRead = (book: Book) => {
    // إذا كان الكتاب محلي، نحصل على محتواه من التخزين المحلي
    if (book.id.startsWith('local_')) {
      const localFileContent = getLocalFile(book.id);
      if (localFileContent) {
        // تحديث رابط الملف للكتاب المحلي
        const bookWithLocalFile = {
          ...book,
          fileUrl: localFileContent // المحتوى الفعلي للملف
        };
        setSelectedBook(bookWithLocalFile);
      } else {
        alert('لم يتم العثور على ملف الكتاب المحلي');
        return;
      }
    } else {
  ...
  const handleUploadComplete = (bookData: any) => {
    ...
    // إعادة تحميل الكتب لإظهار الكتاب الجديد
    const localBooks = getLocalBooks();
    const convertedLocalBooks: Book[] = localBooks.map(localBook => ({
      id: localBook.id,
      title: localBook.title,
    ...
    // إظهار رسالة نجاح
    alert('✅ تم إضافة الكتاب بنجاح! يمكنك الآن العثور عليه في المكتبة.');
  };
