...
const EpubReader: React.FC<EpubReaderProps> = ({ bookId, bookTitle, author, onClose }) => {
  ...
  // تحميل محتوى EPUB من الخادم
  useEffect(() => {
    const loadEpubContent = async () => {
      setLoading(true);
      try {
        // محاولة تحميل محتوى EPUB الحقيقي من الخادم
        const response = await fetch(`http://localhost:5000/api/epub/${bookId}/content`);
        ...
        // في حالة فشل التحميل، نستخدم محتوى تجريبي للعرض
        console.log('Failed to load real EPUB content, using demo content');
