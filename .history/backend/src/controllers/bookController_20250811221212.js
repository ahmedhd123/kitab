...
class BookController {
  ...
  createBook = asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // For now, save to sample data (in production, save to database)
      const book = {
        _id: `book_${Date.now()}`,
        ...bookData,
  ...
  getBookById = asyncHandler(async (req, res) => {
    try {
      ...
      // If database fails, check if this is a recently created book ID
      const bookId = req.params.id;
      
      // For now, return a mock book for recently created books
