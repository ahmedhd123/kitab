  ...
  const uploadBooks = async () => {
    ...
    try {
      ...
      for (const file of uploadedFiles) {
        const savedBook = saveBookLocally(bookMetadata, file);
        savedBooks.push(savedBook);
        console.log('✅ تم حفظ الكتاب:', savedBook.title);
      }
      
      // محاولة إرسال للخادم البعيد (اختياري)
      try {
        ...
        if (response.ok) {
          console.log('✅ تم إرسال الكتاب للخادم أيضاً');
        } else {
          console.log('⚠️ فشل إرسال للخادم، ولكن تم الحفظ محلياً');
        }
      } catch (serverError) {
        console.log('⚠️ خطأ في الاتصال بالخادم، ولكن تم الحفظ محلياً');
      }
      ...
      setTimeout(() => {
        ...
        // إظهار رسالة نجاح
        alert(`✅ تم إضافة ${savedBooks.length} كتاب بنجاح!`);
      }, 1000);
